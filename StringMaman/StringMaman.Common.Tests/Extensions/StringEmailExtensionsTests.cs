...
public class StringEmailExtensionsTests
{
    ...
    [Theory]
    [InlineData("jon_lg92@yahoo")]
    [InlineData("jon_lg92@yahoo.")]
    public void IsValidEmail_WhenEmailIsInvalidStructure_ShouldReturnEmailValidationResult_InvalidStructure(string email)
    {
        // Arrange
        var expected = StringEmailExtensions.EmailValidationResult.InvalidStructure;
        // Act
        var result = email.IsValidEmail();
        // Assert
        Assert.Equal(expected, result);
